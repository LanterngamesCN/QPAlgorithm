...
//初始化权重集合
int CWeight2::init(int weight[], int len)
{
	if (len > MAX_WEIGHT) {
		printf("CWeight::init ERR: len:%d > MAX_WEIGHT:%d\n", len, MAX_WEIGHT);
		return -1;
...
//按权值来随机
int CWeight2::getResultByWeight(int randval)
{
	//int i = GetResultByWeight(weights_, count_);
	//printf(" -> %d\n", indexID_[i]);
	//return indexID_[i];
...
#include <iostream>
ExchangeTy CalcExchangeOrNot3(CWeight2& pool, int randval) {
	pool.shuffleSeq();
	return (ExchangeTy)pool.getResultByWeight(randval);
}
void TestWeightsRatio3(char const* filename) {
	while (1) {
		if ('q' == getchar()) {
			break;
		}
		...
		CWeight2 pool;
		int weight[MaxExchange] = { ratioExC*scale,(100 - ratioExC)*scale };
		int sum = pool.init(weight, MaxExchange);
		std::default_random_engine rengine(time(NULL));
		std::uniform_int_distribution<int> range(1, sum);
		for (int i = 0; i < c; ++i) {
			ExchangeTy ty = CalcExchangeOrNot3(pool, range(rengine));
			if (ty == Exchange) {
				++exC;
			}
			else if (ty == NoExchange) {
